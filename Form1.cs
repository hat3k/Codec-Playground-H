...
namespace Codec_Playground_H
{
    public partial class Form1 : Form
    {
        ...
        private readonly string _settingsFilePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Codec Playground-H Settings.json");
        ...
        private static readonly int[] CBR_Bitrates_MP3 = [8, 16, 24, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320];
        private static readonly int[] ABR_Bitrates_MP3 = [8, 16, 24, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320];
        ...
        // === Unified Cache Instances ===
        private readonly EncodingCache _encodingCache = new();
        private readonly EncoderInfoCache _encoderInfoCache = new();
        ...
        private long _seamlessRequestVersion = 0;
            ...
            try
            {
                ...
                foreach (string audioPath in _settings.AudioFilePaths)
                {
                    ...
                    try
                    {
                        ...
                        {
                            using WaveFileReader wavReader = new(audioPath);
                        ...
                        {
                            (int Channels, int BitsPerSample, int SampleRate, long TotalSamples) = ReadFlacStreamInfo(audioPath);
                            channels = Channels;
                            bitsPerSample = BitsPerSample;
                            sampleRate = SampleRate;
                            durationSec = TotalSamples > 0 && SampleRate > 0
                ...
                else
                {
                    ...
                    Screen screen = Screen.FromPoint(new Point(_settings.Window.X, _settings.Window.Y));
        ...
        private void EnsureMediaInfoDllExists()
        {
            ...
            using Stream? stream = assembly.GetManifestResourceStream("Codec_Playground_H.MediaInfo.dll");
            if (stream == null)
            {
            ...
            catch (Exception ex)
            {
                ...
                _ = MessageBox.Show("Failed to extract MediaInfo.dll. Some features may not work.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        private (string Name, string Version) GetEncoderInfo(string encoderPath)
        {
            ...
            if (_encoderInfoCache.TryGet(encoderPath, out (string Name, string Version) cached))
            {
                return cached;
            }
            ...
            try
            {
                ...
                (string name, string version) result = (name, version);
                _encoderInfoCache.Add(encoderPath, name, version);
                return result;
            }
            catch (Exception ex)
            {
        ...
        private static (int Channels, int BitsPerSample, int SampleRate, long TotalSamples) ReadFlacStreamInfo(string filePath)
        {
            ...
            while (true)
            {
                ...
                _ = fs.Seek(blockSize, SeekOrigin.Current);
                if (isLast)
                {
                    break;
                }
        ...
        private void AddAudioFileFileToList(string audioFileInputPath)
        {
            ...
            try
            {
                ...
                if (ext.Equals(".wav", StringComparison.OrdinalIgnoreCase))
                {
                    using WaveFileReader wavReader = new(audioFileInputPath);
                    channels = wavReader.WaveFormat.Channels;
                    bitsPerSample = wavReader.WaveFormat.BitsPerSample;
                    sampleRate = wavReader.WaveFormat.SampleRate;
                    durationSec = wavReader.TotalTime.TotalSeconds;
                ...
                {
                    (int Channels, int BitsPerSample, int SampleRate, long TotalSamples) = ReadFlacStreamInfo(audioFileInputPath);
                    channels = Channels;
                    bitsPerSample = BitsPerSample;
                    sampleRate = SampleRate;
                    durationSec = TotalSamples > 0 && SampleRate > 0
        ...
        private void ListViewAudioFiles_ItemChecked(object? sender, ItemCheckedEventArgs e)
        {
            ...
            bool wasActive = _currentPlayerState is PlayerState.Playing or PlayerState.Paused;
            if (wasActive)
            {
                _ = Interlocked.Increment(ref _seamlessRequestVersion);
                PausePlaybackForFileSwitch();
                Log($"▶️ Auto-starting playback for new file (was {_currentPlayerState})");
                ...
                StartFileSwitchEncoding(_originalFilePath);
            }
            else
            {
        ...
        private string GetCurrentCommandLineArgs()
        {
            ...
            {
                mode = "CBR";
                bitrate = CBR_Bitrates_MP3[trackBarCBR_MP3.Value];
            }
            else if (radioButtonModeABR_MP3.Checked)
            {
                mode = "ABR";
                bitrate = ABR_Bitrates_MP3[trackBarABR_MP3.Value];
            }
            ...
            bool useQ = checkBoxParameter_q_MP3.Checked;
            if (useQ)
            {
                qValue = Math.Abs(trackBarParameter_q_MP3.Value);
            }

            bool useChannels = checkBoxChannelsModes_MP3.Checked;
            if (useChannels)
            {
                if (radioButtonJointStereo_MP3.Checked)
                {
                    channelMode = "j";
                }
                else if (radioButtonStereo_MP3.Checked)
                {
                    channelMode = "s";
                }
                else if (radioButtonMono_MP3.Checked)
                {
                    channelMode = "m";
                }
            }
            ...
            if (useQ)
            {
                _ = args.Append($"-q {qValue} ");
            }

            if (useChannels && !string.IsNullOrEmpty(channelMode))
            {
                _ = args.Append($"-m {channelMode} ");
            }

            return args.ToString().TrimEnd();
        }
        private string BuildLameArguments(string inputPath, string outputPath, string uiArgs, string presetArgs, bool isPresetSelected)
        {
            StringBuilder args = new();
            if (isPresetSelected)
            {
            ...
            {
                _ = !string.IsNullOrEmpty(uiArgs)
                    ? args.Append($"{uiArgs} \"{inputPath}\" \"{outputPath}\"")
        ...
        private int ParseProgress(string line)
        {
            ...
            return match.Success && int.TryParse(match.Groups[1].Value, out int progress) ? progress : -1;
        }
        private static string GetPreEncodeWavPath(string inputPath, int targetBits, string tempFolder)
        {
            ...
            return Path.Combine(tempFolder, $"preencode_{safeName}_{targetBits}bit____{pathHash}.wav");
        }

        private (string lameInputPath, float[]? preloadedSamples) PrepareLameInput(string inputPath, bool loadSamples, CancellationToken ct)
        {
            ...
            if (ext.Equals(".wav", StringComparison.OrdinalIgnoreCase))
            {
                using WaveFileReader checkReader = new(inputPath);
                int bps = checkReader.WaveFormat.BitsPerSample;
                WaveFormatEncoding enc = checkReader.WaveFormat.Encoding;
            ...
            if (!needsConversion)
            {
                ...
                if (loadSamples)
                {
                    // Load samples from original file
                    if (ext.Equals(".wav", StringComparison.OrdinalIgnoreCase))
                    {
                        using WaveFileReader wavReader = new(inputPath);
                        ISampleProvider provider = wavReader.WaveFormat.BitsPerSample == 24
                    ...
                    {
                        using AudioFileReader audioReader = new(inputPath);
                        samples = ReadAllSamples(audioReader);
                    }
                }
                return (inputPath, samples);
            }

            // Use unified method to get cache path
            string cachedWavPath = GetPreEncodeWavPath(inputPath, targetBits, _tempFolder);
            ...
            if (File.Exists(cachedWavPath) && new FileInfo(cachedWavPath).Length > 0)
            {
                ...
                if (loadSamples)
                {
                    // Load samples from cached WAV
                    using WaveFileReader cachedReader = new(cachedWavPath);
                    ISampleProvider cachedProvider = cachedReader.WaveFormat.BitsPerSample == 24
                        ? new Wave24ToFloatProvider(cachedReader)
                        : cachedReader.ToSampleProvider();
                    samples = ReadAllSamples(cachedProvider);
                    Log($"💾 Original samples loaded from cached WAV: {samples.Length} samples");
                }
                return (cachedWavPath, samples);
            }
            ...
            float[]? convertedSamples = null;

            try
            {
                if (ext.Equals(".wav", StringComparison.OrdinalIgnoreCase))
                {
                    WaveFileReader wavReader = new(inputPath);
                ...
                {
                    AudioFileReader audioReader = new(inputPath);
                ...
                // Load samples if requested (optimization to avoid second disk read)
                if (loadSamples)
                {
                    convertedSamples = ReadAllSamples(sourceProvider);
                    Log($"💾 Original samples loaded during conversion: {convertedSamples.Length} samples");
                }

                WaveFormat targetFormat;
                IWaveProvider waveWriter;

                if (loadSamples && convertedSamples != null)
                {
                    // Use preloaded samples for writing
                    WaveFormat convertedFormat = WaveFormat.CreateIeeeFloatWaveFormat(sampleRate, channels);
                    MemorySampleSource memSource = new(convertedSamples, convertedFormat);
                ...
                using (WaveFileWriter writer = new(tempPreEncodeTmp, targetFormat))
                {
                ...
                return (cachedWavPath, convertedSamples);
            }
            finally
            {
                disposableSource?.Dispose();
                if (File.Exists(tempPreEncodeTmp))
                {
                    try { File.Delete(tempPreEncodeTmp); } catch { }
                }
            }
        }
            ...
            EncodeToTempFileAsync(string inputPath, string cacheKey, string uiArgs, string presetArgs, bool isPreset, CancellationToken ct, long requestVersion = -1)
        {
            ...
            try
            {
                // Use unified PrepareLameInput without loading samples (not needed for seamless encoding)
                (lameInputPath, _) = PrepareLameInput(inputPath, loadSamples: false, ct);
            ...
            {
                Log("⏹️ Encode canceled, killing LAME process");
            ...
            EncodeFileAsync(string inputPath, string cacheKey, string uiArgs, string presetArgs, bool isPreset, CancellationToken ct)
        {
            ...
            if (File.Exists(tempEncodedFile))
            {
                ...
                // Check cache using unified cache
                bool isInCache = _encodingCache.TryGetEncoded(cacheKey, out string? cachedFile, out int cachedDelay);
                ...
                // Calculate delay if needed (outside lock)
                if (isInCache && needsDelayCalc)
                {
                    try
                    {
                        Log($"🔄 Calculating delay for existing file");
                        using (MediaFoundationReader mpegReader = new(tempEncodedFile))
                        {
                        ...
                        // Store calculated delay in cache
                        _encodingCache.AddEncoded(cacheKey, tempEncodedFile, cachedDelay);
                ...
                {
                    (string? ck, string _, string _, bool _) = await GenerateCacheFileNameAndCacheKeyAsync(_originalFilePath, _selectedEncoderPath);
            ...
            try
            {
                // Use unified PrepareLameInput with sample loading
                string lameInputPath;
                (lameInputPath, resultPreloadedSamples) = PrepareLameInput(inputPath, loadSamples: true, ct);
                ...
                _encodingCache.AddEncoded(cacheKey, tempEncodedFile, delay);
        ...
        private async void StartEncodingForPlay(string originalFilePath)
        {
            ...
            if (string.IsNullOrEmpty(_selectedEncoderPath))
            {
                ...
                _ = MessageBox.Show("Please select an encoder first!", "Warning",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            (string? cacheKey, string? uiArgs, string? presetArgs, bool isPreset) = await GenerateCacheFileNameAndCacheKeyAsync(originalFilePath, _selectedEncoderPath);
            _currentCacheKey = cacheKey;
            Log($"🔑 Cache key: {cacheKey}");

            // Use unified cache for fast path
            if (_encodingCache.TryGetEncoded(cacheKey, out string? cachedFile, out int cachedDelay))
            {
            ...
            _currentEncodingTask = Task.Run(async () =>
            {
                ...
                try
                {
                    _ = await EncodeFileAsync(originalFilePath, cacheKey, uiArgs, presetArgs, isPreset, token);
                }
                catch (OperationCanceledException)
                {
        ...
        private async void StartFileSwitchEncoding(string newFilePath)
        {
            ...
            long myVersion = _seamlessRequestVersion;
            (string? cacheKey, string? uiArgs, string? presetArgs, bool isPreset) = await GenerateCacheFileNameAndCacheKeyAsync(newFilePath, _selectedEncoderPath);
            _currentCacheKey = cacheKey;
            Log($"🔑 File switch cache key: {cacheKey}");

            // Fast path: cache hit using unified cache
            if (_encodingCache.TryGetEncoded(cacheKey, out string? cachedFile, out int cachedDelay))
            {
                Log($"📦 File switch: using cached MP3 (delay: {cachedDelay})");
                _encodedFilePath = cachedFile;
                _currentCacheKey = cacheKey;
                _needsReencoding = false;
                _encodingStatus = EncodingStatus.Completed;
                UpdateEncodingUI();
                InitializePlayback(skipStop: true);
                PlayDual();
                UpdateEncoderSettingsReturnedByMILabel();
                Log("✅ File switch applied instantly from cache");
                return;
            }

            // Cache miss — encode in background
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    // Double-check cache inside background task
                    if (_encodingCache.TryGetEncoded(cacheKey, out string? cachedFile2, out int cachedDelay2))
                    {
                        encodedPath = cachedFile2!;
                        delay = cachedDelay2;
                        Log($"📦 File switch: cache hit during encode (delay {delay})");
                    }
                    else
                    {
                        encodedPath = await EncodeToTempFileAsync(newFilePath, cacheKey, uiArgs, presetArgs, isPreset, token, myVersion);
                        if (token.IsCancellationRequested)
                        {
                            return;
                        }

                        Log("🔄 File switch: calculating codec delay...");
                        delay = ComputeDelayForFile(newFilePath, encodedPath);
                        _encodingCache.AddEncoded(cacheKey, encodedPath, delay);
                        Log($"🔧 File switch: delay = {delay} samples");
                    }
                    ...
                    Invoke(() =>
                    {
                        ...
                        {
                            Log("⚠️ File switch superseded — encoded and cached, but not applied");
                        ...
                        PlayDual();
                        UpdateEncoderSettingsReturnedByMILabel();
                        Log("✅ File switch applied — playback started from beginning");
                    });
                }
                catch (OperationCanceledException)
                {
                    Log("⚠️ File switch encode canceled");
                    try
                    {
                        Invoke(() =>
                        {
                ...
                {
                    Log($"❌ File switch failed: {ex.Message}");
                    try
                    {
                        Invoke(() =>
                        {
        ...
        private async void ScheduleSeamlessSwap()
        {
            ...
            (string? cacheKey, string? uiArgs, string? presetArgs, bool isPreset) = await GenerateCacheFileNameAndCacheKeyAsync(audioPath, encoderPath);
        ...
        private void StartSeamlessSwap(string cacheKey, string uiArgs, string presetArgs, bool isPreset)
        {
            ...
            // Fast path: cache hit — swap without progress bar using unified cache
            if (_encodingCache.TryGetEncoded(cacheKey, out string? cachedFile, out int cachedDelay))
            {
                Log($"📦 Seamless: cache hit, swapping directly (delay {cachedDelay})");
                _isSeamlessReencode = true;
                string cachedPath = cachedFile!;
                _ = Task.Run(() =>
                {
                    try
                    {
                        if (myVersion == _seamlessRequestVersion && _playgroundMixer != null)
                        {
                            DecodeAndSwap(cachedPath, cachedDelay, cacheKey);
                        }
                        Invoke(() =>
                        {
                            if (myVersion != _seamlessRequestVersion)
                            {
                                Log("⚠️ Seamless swap superseded (cache hit path)");
                                return;
                            }
                            _encodedFilePath = cachedPath;
                            _needsReencoding = false;
                            _isSeamlessReencode = false;
                            _encodingStatus = EncodingStatus.Completed;
                            UpdateEncodingUI();
                            UpdateEncoderSettingsReturnedByMILabel();
                            Log("✅ Seamless swap applied (from cache)");
                        });
                    }
                    catch (Exception ex)
                    {
                        Log($"❌ Seamless cache swap failed: {ex.Message}");
                        try
                        {
                            Invoke(() =>
                            {
            ...
            // Cache miss - encode to completion
            _isSeamlessReencode = true;
            _encodingStatus = EncodingStatus.Queued;
            UpdateEncodingUI();
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    {
                        encodedPath = await EncodeToTempFileAsync(originalPath, cacheKey, uiArgs, presetArgs, isPreset, token, myVersion);
                        if (token.IsCancellationRequested)
                        {
                            return;
                        }
                    ...
                    // Only swap if this is still the latest request and mixer is available
                    if (myVersion == _seamlessRequestVersion && _playgroundMixer != null)
                    {
                    ...
                    Invoke(() =>
                    {
                        if (myVersion != _seamlessRequestVersion)
                        {
                            Log("⚠️ Seamless swap superseded — encoded and cached, but not applied to mixer");
                            return;
                        }
                        _encodedFilePath = encodedPath;
                        _needsReencoding = false;
                ...
                {
                    Log("⚠️ Seamless encode canceled (application closing)");
                    try
                    {
                        Invoke(() =>
                        {
        ...
        private int CalculateCodecDelay(string originalPath, string encodedPath, int bitrate)
        {
            ...
            MemorySampleSource? memSource = _originalMemorySource;
            ...
            try
            {
                if (origSamplesFromMemory == null)
                {
                    ...
                    if (ext == ".wav")
                    {
                        WaveFileReader wav = new(originalPath);
                        origDisposable = wav;
                    ...
                    {
                        AudioFileReader audio = new(originalPath);
                ...
                using MediaFoundationReader mpegReader = new(encodedPath);
                ISampleProvider mp3Provider = mpegReader.ToSampleProvider();
                WaveFormat mp3Format = mpegReader.WaveFormat;
        ...
        private static int FindSampleOffsetFromArrays(float[] origSamples, float[] encSamples, int channels, int sampleRate, int bitrate)
        {
            ...
            int filterSize = 4;
            if (bitrate is > 0 and <= 56)
            {
                filterSize = 16;
            }
            else if (bitrate is > 0 and <= 64)
            {
                filterSize = 4;
            }
            ...
            Dictionary<int, (int votes, double totalCorr)> allCandidates = [];
            ...
            foreach (int windowStart in windowPositions)
            {
                ...
                int fftSize = 1;
                while (fftSize < actualWindowSize * 2)
                {
                    fftSize <<= 1;
                }
                ...
                List<(int offset, double corr)> windowPeaks = [];
                for (int offset = -searchWindow; offset < searchWindow; offset++)
                {
                    ...
                    if (idxPrev < 0 || idxPrev >= fftSize || idxNext < 0 || idxNext >= fftSize)
                    {
                        continue;
                    }
                ...
                List<(int offset, double corr)> topPeaks = [.. windowPeaks.Take(3)];

                Log($"🪟 Window at {windowStart / (double)sampleRate:F2}s: {topPeaks.Count} peaks");
                foreach ((int offset, double corr) in topPeaks)
                {
                    ...
                    (int votes, double totalCorr) = allCandidates[offset];
                    allCandidates[offset] = (votes + 1, totalCorr + corr);
                }
            }
        ...
        public class Wave24ToFloatProvider : ISampleProvider
        {
            ...
            public int Read(float[] buffer, int offset, int count)
            {
                ...
                {
                    int bytesRead = _reader.Read(_buffer, 0, _buffer.Length); if (bytesRead == 0)
                    {
                        break;
                    }
        ...
        private float[] LoadOriginalToMemory(out WaveFormat originalFormat)
        {
            ...
            if (ext == ".wav")
            {
                WaveFileReader wav = new(_originalFilePath);
            ...
            {
                AudioFileReader audio = new(_originalFilePath);
        ...
        {
            return string.IsNullOrEmpty(_encodedFilePath)
                ? throw new InvalidOperationException("No encoded file path specified for LoadEncodedToMemory")
                : LoadEncodedToMemory(originalFormat, _encodedFilePath);
        }
        ...
        private static float[] GetOriginalDataArray(MemorySampleSource source)
        {
        ...
        private long GetPlaybackPositionBytes()
        {
            ...
            {
                return _playgroundMixer.GetPositionBytes();
            }
        ...
        private long GetPlaybackTotalBytes()
        {
            if (_playgroundMixer != null)
            {
                return _playgroundMixer.GetTotalBytes();
            }
        ...
        private void InitializePlayback(bool skipStop = false, float[]? preloadedSamples = null)
        {
            ...
            try
            {
                ...
                if (preloadedSamples != null)
                {
                    ...
                    if (ext == ".wav")
                    {
                        using WaveFileReader wav = new(_originalFilePath);
                        originalFormat = wav.WaveFormat;
                    }
                    else
                    {
                        using AudioFileReader audio = new(_originalFilePath);
                        originalFormat = audio.WaveFormat;
                    }
                }
                else
                ...
                WaveFormat floatFormat = WaveFormat.CreateIeeeFloatWaveFormat(originalFormat.SampleRate, originalFormat.Channels);
                ...
                if (!string.IsNullOrEmpty(_encodedFilePath) && File.Exists(_encodedFilePath))
                {
                    ...
                    {
                        _ = _encodingCache.TryGetEncoded(_currentCacheKey, out _, out delaySamples);
                    }
                    Log($"📌 Using delay: {delaySamples} samples");

                    // Check decoded cache
                    float[] encData;
                    if (_currentCacheKey != null && _encodingCache.TryGetDecoded(_currentCacheKey, out float[] cached))
                    {
                        encData = cached;
                    ...
                    else
                    {
                        ...
                        if (_currentCacheKey != null)
                        {
                            _encodingCache.AddDecoded(_currentCacheKey, encData);
                            Log($"💾 Decoded cache stored at startup: {encData.Length} samples for {_currentCacheKey}");
                        }
                    }

                    MemorySampleSource encSource = new(encData, floatFormat);
            ...
            catch (Exception ex)
            {
                ...
                Log($"❌ StackTrace: {ex.StackTrace}");
                _ = MessageBox.Show($"Error initializing playback: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        private void DecodeAndSwap(string encodedPath, int delaySamples, string cacheKey)
        {
            CodecPlaygroundMixer? mixer = _playgroundMixer;
            WaveFormat? format = _originalFormat;
            ...
            // Quick check in unified cache
            if (_encodingCache.TryGetDecoded(cacheKey, out float[] cached))
            {
                encData = cached;
            ...
            else
            {
                ...
                _encodingCache.AddDecoded(cacheKey, encData);
                Log($"💾 Decoded cache stored: {encData.Length} samples");
            }

            WaveFormat floatFormat = WaveFormat.CreateIeeeFloatWaveFormat(format.SampleRate, format.Channels);
            MemorySampleSource newEncSource = new(encData, floatFormat);
        ...
        private void PausePlaybackForFileSwitch()
        {
            ...
            Log($"🔄 Playback paused for file switch, state={_currentPlayerState}");
        }
        ...
        private void StopDualPlayback()
        {
            ...
            {
                Log($"ℹ️ Seamless CTS already disposed (expected during shutdown)");
            }
            _seamlessCts = new CancellationTokenSource();
        ...
        {
            if (_isDraggingWaveformSeek || _waveOut == null)
            {
                return;
            }
                    ...
                    if (totalLength > 0)
                    {
                        ...
                        if (progress >= 1.0)
                        {
                            ...
                            else
                            {
                                ...
                                SeekPlaybackToBytes(0);
                                if (_waveOut != null && _currentPlayerState == PlayerState.Playing)
                                {
                                    _waveOut.Play();
                                }
                            }
        ...
        {
            if (!_isDraggingWaveformSeek || e.Button != MouseButtons.Left)
            {
                return;
            }
            ...
            string cacheKey = await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(info.Version) && info.Version != "Unknown")
                    {
                        ...
                        if (encoderVersion.Length > 70)
                        {
                            encoderVersion = encoderVersion[..70];
                        }
                    }
                }
                catch { }
                ...
                if (isPresetSelected)
                {
                    ...
                    if (cleanPresetArgs.Length > 50)
                    {
                        cleanPresetArgs = cleanPresetArgs[..50];
                    }

                    if (string.IsNullOrEmpty(cleanPresetArgs))
                    {
                        cleanPresetArgs = "empty";
                    }

                    string readableName = $"{fileNameWithExt}_{encoderFullName}_PRESET_{cleanPresetArgs}";
                    if (readableName.Length > 130)
                    {
                        readableName = readableName[..130];
                    }
                ...
                foreach (char c in Path.GetInvalidFileNameChars())
                {
                    cleanUiArgs = cleanUiArgs.Replace(c, '_');
                }

                if (string.IsNullOrEmpty(cleanUiArgs))
                {
                    cleanUiArgs = "default";
                }
        ...
        private void ClearCache()
        {
            ...
            // Get all files to delete before clearing cache
            List<string> filesToDelete = _encodingCache.GetAllEncodedFiles();
        ...
        {
            if (_encodingCache.TryGetEncoderSettings(filePath, out string cachedSettings))
            {
            ...
            try
            {
                ...
                _encodingCache.AddEncoderSettings(filePath, result);
                return result;
            }
            catch (Exception ex)
            {
        ...
        {
            labelCBRValue_MP3.Text = CBR_Bitrates_MP3[trackBarCBR_MP3.Value].ToString();
            Log($"📊 CBR trackbar: {labelCBRValue_MP3.Text}");
            OnSettingsChanged();
        }
        private void TrackBarABR_Scroll(object? sender, EventArgs e)
        {
            labelABRValue_MP3.Text = ABR_Bitrates_MP3[trackBarABR_MP3.Value].ToString();
        ...
        private void ButtonSaveUserPreset_Click(object? sender, EventArgs e)
        {
            ...
            int presetNumber;
            if (button == buttonSaveUserPreset1)
            {
                presetNumber = 1;
            }
            else if (button == buttonSaveUserPreset2)
            {
                presetNumber = 2;
            }
            else if (button == buttonSaveUserPreset3)
            {
                presetNumber = 3;
            }
            else if (button == buttonSaveUserPreset4)
            {
                presetNumber = 4;
            }
            else if (button == buttonSaveUserPreset5)
            {
                presetNumber = 5;
            }
            else if (button == buttonSaveUserPreset6)
            {
                presetNumber = 6;
            }
            else
        ...
        private void ButtonUserPresetClear_Click(object? sender, EventArgs e)
        {
            ...
            int presetNumber;
            if (button == buttonUserPreset1Clear)
            {
                presetNumber = 1;
            }
            else if (button == buttonUserPreset2Clear)
            {
                presetNumber = 2;
            }
            else if (button == buttonUserPreset3Clear)
            {
                presetNumber = 3;
            }
            else if (button == buttonUserPreset4Clear)
            {
                presetNumber = 4;
            }
            else if (button == buttonUserPreset5Clear)
            {
                presetNumber = 5;
            }
            else if (button == buttonUserPreset6Clear)
            {
                presetNumber = 6;
            }
            else
            ...
            switch (presetNumber)
            {
                case 1: textBoxUserPreset1.Text = ""; break;
                case 2: textBoxUserPreset2.Text = ""; break;
                case 3: textBoxUserPreset3.Text = ""; break;
                case 4: textBoxUserPreset4.Text = ""; break;
                case 5: textBoxUserPreset5.Text = ""; break;
                case 6: textBoxUserPreset6.Text = ""; break;
            }
        ...
        private void SelectRandomAudioFile()
        {
            ...
            if (listViewAudioFiles.Items.Count == 1)
            {
                ...
                ListViewItem item = listViewAudioFiles.Items[0];
                item.Checked = true;
            ...
            ListViewItem selectedItem = listViewAudioFiles.Items[selectedIndex];
        ...
        private void ABTestPresets(int count)
        {
            ...
            for (int i = 1; i <= count; i++)
            {
                ...
                if (radio != null && !string.IsNullOrEmpty(args))
                {
                    ...
                    if (radio.Checked)
                    {
                        currentIndex = presetRadios.Count - 1;
                    }
                }
            }
            ...
            RadioButton selectedRadio = presetRadios[newIndex];
        ...
        private async Task CheckForUpdatesAsync()
        {
            ...
            try
            {
                ...
                _ = Invoke(() => labelNoUpdates.Visible = false);
        ...
        private async Task<GitHubRelease?> GetLatestReleaseAsync()
        {
            try
            {
                ...
                _ = httpClient.DefaultRequestHeaders.UserAgent.TryParseAdd("Codec-Playground-H/1.0");
        ...
        private void Form1_FormClosing(object? sender, FormClosingEventArgs e)
        {
            ...
            try
            {
                ...
                // Cancel primary encoding task (Play button)
                _encodingCts?.Cancel();
                if (_currentEncodingTask != null && !_currentEncodingTask.IsCompleted)
                {
                    Log($"⏳ Waiting for encoding task to finish...");
                    try { _ = _currentEncodingTask.Wait(2000); } catch { }
                }

                // StopDualPlayback cancels _seamlessCts and disposes waveOut
                StopDualPlayback();

                // Give WASAPI time to release audio device and temp files
                for (int i = 0; i < 20; i++)
                {
            ...
            finally
            {
                ...
                {
                    Log($"❌ CleanupTempFiles error: {ex.Message}");
                }
            }
            Log($"🔚 Form closed");
        }
        private void CleanupTempFiles()
        {
            ...
            try
            {
                // Get all encoded files from cache
                filesToDelete.AddRange(_encodingCache.GetAllEncodedFiles());
                _encodingCache.Clear();

                if (Directory.Exists(_tempFolder))
                {
                    string[] tempFiles = Directory.GetFiles(_tempFolder, "*.mp3");
                    ...
                    string[] preEncodeFiles = Directory.GetFiles(_tempFolder, "preencode_*.wav");
                    Log($"📊 Found {preEncodeFiles.Length} pre-encode WAV files in temp");
                    foreach (string file in preEncodeFiles)
                    {
                        if (!filesToDelete.Contains(file))
                        {
                            filesToDelete.Add(file);
                    ...
                    string[] tmpFiles = Directory.GetFiles(_tempFolder, "preencode_*.wav.tmp");
        ...
        public class EncodingCache
        {
            ...
            {
                public string EncodedPath { get; set; } = string.Empty;
                public int Delay { get; set; }
                public float[]? DecodedSamples { get; set; }
            }

            private readonly Dictionary<string, CacheEntry> _entries = [];
            private readonly Dictionary<string, string> _encoderSettingsCache = [];
            private readonly Lock _lock = new();

            /// <summary>Try to get encoded file path and delay from cache.</summary>
            public bool TryGetEncoded(string key, out string? filePath, out int delay)
            {
                lock (_lock)
                {
                    filePath = null;
                    ...
                    if (_entries.TryGetValue(key, out CacheEntry? entry) &&
                        !string.IsNullOrEmpty(entry.EncodedPath) &&
                        File.Exists(entry.EncodedPath))
                    {
                        filePath = entry.EncodedPath;
                        delay = entry.Delay;
                        return true;
                    }
                    return false;
            ...
            /// <summary>Add encoded file path and delay to cache.</summary>
            public void AddEncoded(string key, string filePath, int delay)
            {
                lock (_lock)
                {
                    if (!_entries.TryGetValue(key, out CacheEntry? entry))
                    ...
                    entry.EncodedPath = filePath;
                    entry.Delay = delay;
                }
            }

            /// <summary>Try to get decoded samples from cache.</summary>
            public bool TryGetDecoded(string key, out float[] samples)
            {
                lock (_lock)
                {
                    samples = null!;
                    if (_entries.TryGetValue(key, out CacheEntry? entry) && entry.DecodedSamples != null)
                    {
                        samples = entry.DecodedSamples;
                        return true;
                    }
                    return false;
                }
            ...
            /// <summary>Add decoded samples to cache.</summary>
            public void AddDecoded(string key, float[] samples)
            {
                lock (_lock)
                {
                    if (!_entries.TryGetValue(key, out CacheEntry? entry))
                    ...
                    entry.DecodedSamples = samples;
                }
            }

            /// <summary>Try to get encoder settings from cache.</summary>
            public bool TryGetEncoderSettings(string filePath, out string settings)
            {
                lock (_lock)
                {
                    if (_encoderSettingsCache.TryGetValue(filePath, out string? s))
                    {
                        settings = s;
                        return true;
                    }
            ...
            /// <summary>Add encoder settings to cache.</summary>
            public void AddEncoderSettings(string filePath, string settings)
            {
                lock (_lock)
                {
                    _encoderSettingsCache[filePath] = settings;
                }
            }

            /// <summary>Get all encoded file paths for cleanup.</summary>
            public List<string> GetAllEncodedFiles()
            {
                lock (_lock)
                {
                    return [.. _entries.Values
                        .Where(e => !string.IsNullOrEmpty(e.EncodedPath))
                        .Select(e => e.EncodedPath)];
                }
            }
            ...
            public void Clear()
            {
                ...
                {
                    _entries.Clear();
                    _encoderSettingsCache.Clear();
                }
            }
        }

        public class EncoderInfoCache
        {
            private readonly Dictionary<string, (string Name, string Version)> _cache = [];
            private readonly Lock _lock = new();

            public bool TryGet(string encoderPath, out (string Name, string Version) info)
            {
                lock (_lock)
                {
                    return _cache.TryGetValue(encoderPath, out info);
                }
            }

            public void Add(string encoderPath, string name, string version)
            {
                lock (_lock)
                {
                    _cache[encoderPath] = (name, version);
                }
            }
